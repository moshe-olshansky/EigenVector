...
int main(int argc, char *argv[]) {
	...
	string hg38("hg38");
	string genome;
	i = getHiCInfo(fname,chroms,chrLen,genome);
	if (i < 0) {
		cerr << "Error! File " << fname << " cannot be opened for reading" << endl;
		exit(EXIT_FAILURE);
	}
	if (genome != hg19 && genome != hg38) if (verb) {
		cout << genome;
...
//	loop accross all chromosomes
		...
		ftime(&t0);
        	records = straw(ob, norm, fname, chroms.at(i), chroms.at(i), unit, binsize);
		...
		int N = (int) ceil(chrLen.at(i)/((double) binsize));
